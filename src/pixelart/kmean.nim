...
var tmpAvgColors = newSeq[Rgb]()
var sucAvgColors = newSeq[Rgb]()
  ...
  proc setUpUnionFindTree(image:Image): MonoidUnionFind[SizeAndColor] =
    ...
    for x in 0..<w:
      for y in 0..<h:
        ...
        let c = image.getColors(getRect(lt,rb)).median()
        pixelImage[x,y] = c
        colors[xyToI(x,y)] = (1,pixelImage[x,y])
        tmpAvgColors &= (c.r.float,c.g.float,c.b.float)
  ...
  stopwatch:
    ...
    for i in 17..17:
      ...
      if ok:
        ...
        f.close()
      if ok:
        let f = fmt"plot/colors-{target}.txt".open(fmWrite)
        f.writeLine (($sucAvgColors)[1..^1]).replace("r: ","").replace("g: ","").replace("b: ","") & ","
        f.close()
